        ...
        <body>
            ...
            <script>
                ...
                $(document).ready(function () {
                    ...
                    // Filter toggle handler - also save to localStorage
                ...
                function fetchMetrics() {
                    ...
                    const endpoint = `/${basePath}/monitoring/live`;  // Changed: added / at the start

                    const url = viewerId
                        ? `${endpoint}?viewerid=${viewerId}&filtersystem=${filterSystem}`
                        : `${endpoint}?filtersystem=${filterSystem}`;

                    console.log('Fetching from:', url);  // Keep this to verify
                    $.ajax({
                        url: url,
                        method: 'GET',
                        dataType: 'json',
                ...
                function openModal(chartType, title) {
                    ...
                    // Clone only the data arrays (not the entire object)
                    const clonedData = {
                    ...
                    // Recreate options without using spread (to avoid resolver issues)
                    const modalOptions = {
                        responsive: true,
                        maintainAspectRatio: false,
                    ...
                    // Add y1 scale for dbMemory chart
                    ...
                    // Add memory scale for memory chart
                    if (chartType === 'memory') {
                    ...
                    // Create modal chart with fresh canvas context
